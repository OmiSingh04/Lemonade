...
public class InfoEvent extends ListenerAdapter{
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if(message.startsWith(App.SERVER_PREFIX + "INFO")) {
			...
			info.addField("Developer","Omi Singh",true);
		...
		else if(message.startsWith(App.SERVER_PREFIX + "SPAM"))
		{
			...
			int n = Integer.parseInt(message.substring(8));
			while(--n>=0)
			event.getChannel().sendMessage(name).queue();
		}
		else if(message.startsWith("LM.SEND")) {
			EmbedBuilder builder = new EmbedBuilder();
			builder.setImage("https://http.cat/500");
			builder.setDescription("Image");
